...
export default function AttendanceGrid({
  ...
  const isPresent = (NIS, day) => {
    const result = attendanceList.find(
      (item) => item.day == day && item.NIS == NIS
    );
  ...
  const onMarkAttendance = (day, NIS, presentStatus) => {
    ...
    if (presentStatus) {
      ...
      GlobalApi.MarkAttendance(data, kegiatan, NIS, selectedMonth).then(
        (resp) => {
  ...
  return (
    <div
      className="ag-theme-quartz text-sm w-[320px] md:w-full" // applying the grid theme
